...
contract Domains is ERC721URIStorage {
    ...
    function price(string calldata name) public pure returns(uint) {
        ...
        require(len > 0);
        if (len == 3) {
            return 5 * 10**17; // 5 MATIC = 5 * 10**17 (18 decimals)
        } else if (len == 4) {
            return 3 * 10**17;
        } else {
            return 1 * 10**17;
    ...
    function register(string calldata name) public payable {
        ...
        // JSON metadata of our NFT.
        // We do this by combining strings and encoding as base64
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Cool Name Service", "image": "data:image/svg+xml;base64,',
        ...
        // set the nft data -- in this case the Json blob w/ our domain's info!
